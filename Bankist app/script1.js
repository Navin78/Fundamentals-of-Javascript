...
const account1 = {
  ...
  movementsDates:[
    ...
    "2023-01-26T23:36:17.929Z",  // years month date 
...
//----------------------------------DATES METHOD-----------------------------------//
  ...
  // we can also use the the options like 
  //1.currency 
  //2.style some of the use cases
...
// set timer for the login and the logout process 

const startlogoutTimer=function(){
const tick=function() // create the tick function inside the values can be altered 
{
  const hour=String(Math.trunc(time/60)).padStart(2,0);
  const min=String(Math.trunc(time /60 )).padStart(2,0);
  const sec=String(time % 60 ).padStart(2,0); 

  // In each call timer will display in the UI
  ...
  if(time===0)
{
  ...
  labelWelcome.textContent = 'Login to get into started';
...
tick();
const timer=setInterval(tick,1000);    // 10000 milli seconds 
return timer;               
};
...
//if the user login it can goes to the front page without the login   // Fake login   
...
//----------------------------DATES--------------------------------//
    ...
    const now=new Date();
  const day=`${now.getDay()}`.padStart(2,0);   // pad start defines the (2,0) 2 is length of the characters ,0 is the inital values 
...
btnLogin.addEventListener("click", function (e) {
  ...
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    ...
    containerApp.style.opacity = 100;
          ...
          // Internationalizing time and date format Expermenting API
           const now=new Date();
          const options=
          {
            hour:'numeric',
            ...
            labelDate.textContent=new Intl.DateTimeFormat('en-UK',options,currentAccount).format(now);
...
// timer will change one acc into another acc timer will refresh the timer goes on the starting time 
...
btnLoan.addEventListener('click', function (e) {
  ...
  //set timeout means some particular time will display the contents and the values  
...
console.log((new Date ( 3 * 24 * 60 * 60 *1000 ))); // 3days 24hours 60min 60sec 10000 milliseconds 
...
setTimeout(() => console.log('HEY MAN'), 3000);
